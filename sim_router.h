...
//---------------------------------------------------------------------------//
		...
		void add_routing(long a, long b, long c) 
				{   
					for(int vc=0; vc<configuration::ap().virtual_channel_number();vc++)
					   routing_[a][b].push_back(VC_type(c,vc));				
				}
...
class sim_router_template {
	...
	public:
		...
		void Adaptive_VN_algorithm(const add_type & des_t, const add_type & sor_t, 
				long s_ph, long s_vc);
		void Adaptive_TM_algorithm(const add_type & des_t, const add_type & sor_t, 
				long s_ph, long s_vc);
